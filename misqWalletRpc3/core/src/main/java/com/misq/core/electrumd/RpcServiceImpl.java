    ...
    public CompletableFuture<List<Utxo>> listUnspent() {
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            for (TransactionOutput txo : rpcSpecificUtxos) {
                ...
                x.privateKey = client.getAddressPrvkey(txo.address);
    ...
    public CompletableFuture<String> createRawTransaction(List<Utxo> inputs, List<Utxo> outputs) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            for (Utxo txi : inputs) {
                ...
                inputsMap.put("privkey", txi.privateKey);
            ...
            Map<String, List<Object>> jsonO = new HashMap<>();
            jsonO.put("inputs", inputsArray);
            jsonO.put("outputs", outputsArray);
            Map<String, String> retVal = client.serialize(jsonO);
            String signed = retVal.get("hex");
            return signed;
        });
    }

    public String signTransaction(String txHex, String privKey) {
        Map<String, Object> retVal = client.signTransaction(txHex, privKey);
        return (String) retVal.get("hex");
    }
