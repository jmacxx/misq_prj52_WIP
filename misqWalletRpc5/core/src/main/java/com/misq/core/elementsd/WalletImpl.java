...
public class WalletImpl implements Wallet {
    ...
    public CompletableFuture<List<Utxo>> listUnspent() {
        ...
        return rpcService.listUnspent().handle((rpcSpecificUtxos,b) -> {
            ...
            for (TransactionOutput txo : rpcSpecificUtxos) {
                retVal.add(new Utxo().initFromUnspent(txo.txId, txo.vout, txo.scriptPubKey, txo.amount));
            }
            return retVal;
        });
    }
    ...
    public EnumSet<Capability> getCapability() {
        return EnumSet.of(Capability.SEND_AND_RECEIVE, Capability.CHAINED_TX, Capability.MULTISIG, Capability.TOKENS);
    }
}
