...
public class MultisigTest {
    ...
    public void createSignedDepositTx() throws IOException {
        ...
        transactionOutputs.add(new Utxo().initForOutput(multisigDepositInfo.address, totalAmount.toPlainString()));
    ...
    public void createSignedPayoutTx() throws IOException {
        ...
        List<Utxo> transactionInputs = new ArrayList<>();
        transactionInputs.add(new Utxo().initFromUnspent(depositTxId, 0, ""));   // vout from multisig is always 0

        // bob pays the miner fee
        String spendingAmount = (Coin.parseCoin(aliceFundingAmount).add(Coin.parseCoin(bobFundingAmount).subtract(Coin.valueOf(1000)))).toPlainString();
        ...
        List<Utxo> transactionOutputs = new ArrayList<>();
        transactionOutputs.add(new Utxo().initForOutput(alicePayoutAddress, bobFundingAmount));
        transactionOutputs.add(new Utxo().initForOutput(bobPayoutAddress, payoutMinusFee));
        System.out.println("payouts: " + transactionOutputs.toString());

        CompletableFuture<String> signedTx = aliceWallet.createRawTransaction(transactionInputs, transactionOutputs)
                .thenCompose((a) -> { return aliceWallet.signRawTransactionWithKey(a, alicePriv, this.depositTxId, 0, multisigDepositInfo.scriptPubKey, multisigDepositInfo.witnessScript, spendingAmount);})
                .thenCompose((b) -> { return aliceWallet.signRawTransactionWithKey(b, bobPriv, this.depositTxId, 0,  multisigDepositInfo.scriptPubKey, multisigDepositInfo.witnessScript, spendingAmount);})
                .handle((signed, ex) -> {
